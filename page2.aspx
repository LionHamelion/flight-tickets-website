...
<head>
    ...
    function initializeCities() {
        ...
        if (allCities.length > 1) {
            ...
            // Оновлення списків, щоб уникнути дублювання
            updateOptions(arrival, departure.value, arrival.value);
            updateOptions(departure, arrival.value, departure.value);
        }
    }
...
<body>
    ...
    <div class="container">
        ...
        <div>
            ...
            <asp:TextBox ID="NameTextBox" runat="server" CssClass="form-control" required="true"/>
        </div>
        <div class="photoUpload">
            <label>ФОТОГРАФІЯ (JPEG/PNG, min 100x150, max 200x300):</label><br />
            <asp:FileUpload ID="PhotoUpload" runat="server" CssClass="form-control" required="true"/>
        </div>
        <div>
    <label for="DepartureCity">ЗВІДКИ:</label>
    <asp:DropDownList ID="DepartureCity" runat="server" onchange="updateCityList()" required="true"/>
        </div>
        <div>
    <label for="ArrivalCity">КУДИ:</label>
    <asp:DropDownList ID="ArrivalCity" runat="server" onchange="updateCityList()" required="true"/>
        </div>
        <div>
        ...
        <div class="flightDate">
            ...
            <asp:TextBox ID="FlightDate" runat="server" CssClass="form-control" TextMode="Date" required="true"/>
        </div>
        <div class="flightDate">
            <label>Дата зворотнього рейсу:</label>
            <asp:TextBox ID="ReturnDate" runat="server" CssClass="form-control" TextMode="Date" required="true"/>
        </div>
        <div class="buttons">
        <asp:Button ID="PrevBtn" UseSubmitBehavior="false" OnClientClick="hideSubmitButtons()" CausesValidation="False" runat="server" Text="НАЗАД" OnClick="PrevBtn_Click" />
